...
int maxImg = 3;
int bailarinesIndex = 0; 

PImage [] imagesBailarines = new PImage [maxImg];

PImage fondoInicio, fondoJugando, tituloInicio, tituloPerder, tituloGanar, instrucciones, botonInicio, botonJugar, botonReintentar, radio;
...
amorSalvaje.loop();
  ...
  fondoJugando = loadImage ("imagenes/fondoJugando.jpg");
  ...
  //bailarines = loadImage("imagenes/parejitaBailando.png");
  radio = loadImage("imagenes/radio.png");
  //resize bailarines
   //bailarines.resize(400, 450);
  for (int i = 0; i<imagesBailarines.length; i++){
    imagesBailarines[i]= loadImage("imagenes/parejitaBailando_" + i + ".png"); 
    //imagesBailarines[i].resize(400, 450);
  }
...
void draw() {
  ...
  if (estado==2) {
    ...
    image(imagesBailarines[bailarinesIndex],1100, 100);
    imagesBailarines[bailarinesIndex].resize(400, 450);
...
void contactStarted(FContact contacto) {
  ...
  if ( body1.getName() != null && body2.getName() !=null && body1.getName() != body2.getName()) {
    ...
    progreso++;
    empezarTiempo=true;
    //bailarinesIndex = (bailarinesIndex+1) % imagesBailarines.length;
    
    bailarinesIndex ++;
    if(bailarinesIndex > 0){
       bailarinesIndex =0;
    }
